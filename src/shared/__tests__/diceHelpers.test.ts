...
describe("dice helpers", () => {
  ...
  describe("isSuccess", () => {
    it.each([Result.SUCCESS, Result.HARD_SUCCESS, Result.EXTREME_SUCCESS])
      ("should return true [%d]", function(input) {
      expect(isSuccess(input)).toEqual(true)      
    })

    it.each([Result.FAIL, Result.FUMBLE])
      ("should return false [%d]", function(input) {
